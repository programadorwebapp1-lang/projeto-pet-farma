...
import { brinquedos } from "./mockBrinquedos";
...
function HeaderRemedios({ onBuscar = undefined, carrinho, onCarrinhoClick }) {
  ...
  // Sugestões de produtos (brinquedos + remédios)
  const sugestoes = (busca.trim() === "") ? [] : [
    ...brinquedos.filter(p => p.nome.toLowerCase().includes(busca.toLowerCase())).map(p => ({
      id: `brinquedo-${p.id}`,
      nome: p.nome,
      tipo: "Brinquedo",
      imagem: p.imagem
    })),
    ...remedios.filter(p => p.nome.toLowerCase().includes(busca.toLowerCase())).map(p => ({
      id: `remedio-${p.id}`,
      nome: p.nome,
      tipo: "Remédio",
      imagem: p.foto
  ...
  return (
    <header className="w-full px-4 py-3 bg-green-700 flex items-center justify-between flex-wrap gap-2 relative">
      <span className="min-w-[180px]">Pet Farma - Brinquedos</span>
      <form
      ...
      >
        <div className="relative w-full max-w-md">
          <input
            ...
            className="w-full rounded-full border-none px-4 py-2 pr-10 text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-orange-300 placeholder:text-gray-400  shadow-sm search-input-custon"
            autoComplete="off"
          />
            ...
            <div className="absolute left-0 mt-2 w-full bg-white border border-gray-200 rounded-xl shadow-lg z-10">
              {sugestoes.map(sugestao => (
                <button
                  key={sugestao.id}
                  type="button"
                  className="flex items-center w-full px-4 py-3 text-lg hover:bg-gray-100 text-left gap-2 cursor-pointer"
                  onMouseDown={() => {
                    setBusca(sugestao.nome);
                    setShowSugestoes(false);
                  }}
                >
                  <img src={sugestao.imagem} alt={sugestao.nome} className="w-7 h-7 rounded bg-gray-100 border mr-2" />
...
export default function CarrinhoBrawer() {
  const {
    carrinho,
    remover,
    alterarQuantidade,
            ...
            <div className="flex-1 overflow-y-auto p-5">
              {carrinho.length === 0 ? (
                ...
                <ul className="space-y-5">
                  {carrinho.map((item) => {
                    let imagemProduto = "";
                    if ('imagem' in item.produto) {
                      imagemProduto = item.produto.imagem;
                    } else if ('foto' in item.produto) {
                      imagemProduto = item.produto.foto;
                    }
                    return (
                      <li key={item.produto.id} className="flex items-center gap-3 bg-blue-50 rounded-xl p-3 hover:bg-blue-100 transition-colors duration-150 cursor-pointer">
                        <img src={imagemProduto} alt={item.produto.nome} className="w-14 h-14 rounded-lg object-contain bg-white border" />
